...
export function calculateGrowthVsNPeriodAgo(
  inputDataArraySortBasedOnTimeDescending,
  nPeriodAgo
) {
  if (
    inputDataArraySortBasedOnTimeDescending.length > nPeriodAgo &&
    nPeriodAgo != 0
  ) {
    var valueCurrent = parseFloat(inputDataArraySortBasedOnTimeDescending[0]);
    var valueNPeriodAgo = parseFloat(
      inputDataArraySortBasedOnTimeDescending[nPeriodAgo]
    );
    var valueGrowth = valueCurrent - valueNPeriodAgo;
    var valueGrowthFormatted = "(" + numeral(valueGrowth).format("0.00a") + ")";
    var valueGrowthRate =
      valueNPeriodAgo != 0 ? valueGrowth / valueNPeriodAgo : 1.0;
    var valueGrowthRateFormatted =
      (valueGrowth < 0 ? "⬇" : "⬆") + (valueGrowthRate * 100).toFixed(2) + "%";
    return [
      valueGrowth,
      valueGrowthFormatted,
      valueGrowthRate,
      valueGrowthRateFormatted,
    ];
  } else {
...
export function dynamicColor(value, positive_is_bad) {
...
export function humanReadableNumber(value, is_human_readable) {
...
export function percentageNumber(value, is_percentage_number) {
